...
PropertiesPanel.prototype._bindListeners = function(container) {
  ...
  var handleChange = function handleChange(event) {
    updateCodeCompletion(self, event);
...
//custom
var currentCodeCompletionId = null;
var currentCodeCompletionElement = null;
var lastValue = "";

function setCodeCompletionVisibility(visibility){
    console.log("setting visibility: " + visibility);
    var element = document.getElementById("code-completion");
    element.style.visibility = visibility;
}

function updateCodeCompletion(propertiesPanel, event){
    var codeCompletionFields = ["camunda-assignee"];
    var inputElement = event.srcElement;
    codeCompletionFields.forEach(function(element){
        if(event.srcElement.id === element){
            if(element === currentCodeCompletionId){
                if(element === "camunda-assignee"){
                   setCodeCompletionVisibility("visible");
                   updateAssigneeCodeCompletion(inputElement);
               }
            }
            else if(currentCodeCompletionId === null){
               ...
               currentCodeCompletionElement = document.createElement("div");
               currentCodeCompletionElement.className = "codeCompletion";
               currentCodeCompletionElement.style.zIndex = 100;
               currentCodeCompletionElement.style.top = inputElementRect.top + inputElementRect.height;
               currentCodeCompletionElement.style.left = inputElementRect.left;
               currentCodeCompletionElement.style.width = inputElementRect.width;
               currentCodeCompletionElement.id = "code-completion";
               
               inputElement.addEventListener("blur", function(){
                   console.log("blur");
                    setTimeout(function(){
                        setCodeCompletionVisibility("hidden");    
                    }, 200); 
               }); 
               
               document.body.appendChild(currentCodeCompletionElement);
               currentCodeCompletionId = element;
...
function updateAssigneeCodeCompletion(inputElement){
    if(lastValue !== inputElement.value){
        currentCodeCompletionElement.innerHTML = "";
        getUsers(inputElement.value, function(response){
            var users = response.user_service.users;
            users.forEach(function(user){
                var child = document.createElement("div");
                child.className = "codeCompletionLine";
                child.innerHTML = user.name;
                child.addEventListener("click", function(){
                    inputElement.value = user.name;
                    emitDOMEvent(inputElement, "change");
                    setCodeCompletionVisibility("hidden");
                });
                currentCodeCompletionElement.appendChild(child);
            });
            if(users.length === 0){
                var child = document.createElement("div");
                child.className = "codeCompletionLine";
                child.innerHTML = "keine Treffer";
                currentCodeCompletionElement.appendChild(child);
            }
        });
...
function getUsers(query, callback){
    httpGet("http://test-cdn.abas.de/hska/bpmn/users?q=" + query, "json", callback);
}
...
function emitDOMEvent(element, eventName){
    var event;
